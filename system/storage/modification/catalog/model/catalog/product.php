...
class ModelCatalogProduct extends Model {
	...
	public function getProductCalendars($product_id = 0){
		...
		while($start < date('Y-m-d', strtotime('+6 month'))){
			
			$Calendar[$start] = array();
			
			foreach ($results as $result) {
				
				//Если выборка только по одному диджею - то только его
				if($product_id == 0 OR $product_id == $result['product_id']){
					$Calendar[$start][$result['product_id']] = array(
																'busy'      => 3,
																'busy_name'      => $busy[3],
																'text'       => '',
				...
				/*					
				if(!isset($produsts[$result['product_id']])){
					$produsts[$result['product_id']] = array(
			...
			$start = date('Y-m-d', strtotime('+'.++$i.' day'));
			
		}
	
		if($product_id > 0){
			$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "calendar c
									  ...
									  WHERE p.status=1 AND c.`date` >= '".date('Y-m-d')."' AND c.product_id = '" . (int)$product_id . "' ORDER BY c.`date` DESC");
		}else{
			$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "calendar c
									  LEFT JOIN " . DB_PREFIX . "product p ON (p.product_id = c.product_id)
									  LEFT JOIN " . DB_PREFIX . "product_description pd ON (pd.product_id = c.product_id AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "')
									  WHERE p.status=1 AND c.`date` >= '".date('Y-m-d')."' /*AND (`busy`='3' OR `busy`='2')*/ ORDER BY c.`date` DESC");
		}

		
		foreach ($query->rows as $result) {
			$Calendar[date('Y-m-d', strtotime($result['date']))][$result['product_id']]['busy'] = $result['busy'];
			$Calendar[date('Y-m-d', strtotime($result['date']))][$result['product_id']]['busy_name'] = $busy[$result['busy']];
			$Calendar[date('Y-m-d', strtotime($result['date']))][$result['product_id']]['text'] = $result['text'];
			
			//Если это общий календарь то занятых вообще удаляем из списка
			if($product_id == 0 AND $result['busy'] == 1){
				unset($Calendar[date('Y-m-d', strtotime($result['date']))][$result['product_id']]);
			}
		}
		...
		foreach($Calendar as $index => $row){
			...
			while(count($products)){
				...
				$Calendar[$index][$id] = $products[$id];
				unset($products[$id]);
			}
		}
		
		//echo "<pre>";print_r(var_dump($Calendar));echo "</pre>";
		//die();
	...
	public function getProductImageFirst($product_id) {
	...
	public function getProduct($product_id) {
		...
		if ($query->num_rows) {
			return array(
				...
				'sku'              => $query->row['sku'],
					'instagram_id'              => $query->row['instagram_id'],
					'instagram_code'              => $query->row['instagram_code'],
				'upc'              => $query->row['upc'],
				'ean'              => $query->row['ean'],
				'jan'              => $query->row['jan'],
				'isbn'             => $query->row['isbn'],
