...
class ModelCatalogProduct extends Model {
	...
	public function getProductCalendars($product_id){
	
		$Calendar = array();

		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "calendar WHERE product_id = '" . (int)$product_id . "' ORDER BY `date` DESC");

		foreach ($query->rows as $result) {
			$Calendar[$result['date']] = array(
	...
	public function addProduct($data) {
		...
		if (isset($data['instagram_code'])) {
			$this->db->query("UPDATE " . DB_PREFIX . "product SET instagram_code = '" . $this->db->escape($data['instagram_code']) . "' WHERE product_id = '" . (int)$product_id . "'");
			$this->db->query("UPDATE " . DB_PREFIX . "product SET instagram_id = '" . $this->db->escape($data['instagram_id']) . "' WHERE product_id = '" . (int)$product_id . "'");
		}
		...
		if (isset($data['product_image'])) {
			foreach ($data['product_image'] as $product_image) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_image SET product_id = '" . (int)$product_id . "',
								...
								`desc` = '" . (isset($product_image['desc']) ? 1 : 0) . "',
								`mobil` = '" . (isset($product_image['mobil']) ? 1 : 0) . "',
	...
	public function editProduct($product_id, $data) {
		...
		if (isset($data['product_image'])) {
			foreach ($data['product_image'] as $product_image) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_image SET product_id = '" . (int)$product_id . "',
								`desc` = '" . (isset($product_image['desc']) ? 1 : 0) . "',
								`mobil` = '" . (isset($product_image['mobil']) ? 1 : 0) . "',
								image = '" . $this->db->escape($product_image['image']) . "', sort_order = '" . (int)$product_image['sort_order'] . "'");
			}
		}
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "calendar WHERE product_id = '" . (int)$product_id . "'");
		foreach ($data['calendar'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "calendar SET product_id = '" . (int)$product_id . "',
							 date = '" . $this->db->escape($value['date']) . "',
	...
	public function getProducts($data = array()) {
		...
		$sort_data = array(
			...
			$sql .= " ORDER BY p2c.category_id, " . $data['sort'];
		} else {
