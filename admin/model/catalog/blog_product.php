...
class ModelCatalogBlogProduct extends Model {
	public function addProduct($data) {
		...
		if (isset($data['template'])) {
			$this->db->query("UPDATE " . DB_PREFIX . "blog_product SET template = '" . $this->db->escape($data['template']) . "' WHERE blog_product_id = '" . (int)$blog_product_id . "'");
		}
		...
		if (isset($data['product_attribute'])) {
			foreach ($data['product_attribute'] as $product_attribute) {
				if ($product_attribute['attribute_id']) {
					// Removes duplicates
					$this->db->query("DELETE FROM " . DB_PREFIX . "blog_product_attribute WHERE blog_product_id = '" . (int)$blog_product_id . "' AND attribute_id = '" . (int)$product_attribute['attribute_id'] . "'");

					foreach ($product_attribute['product_attribute_description'] as $language_id => $product_attribute_description) {
						$this->db->query("DELETE FROM " . DB_PREFIX . "blog_product_attribute WHERE blog_product_id = '" . (int)$blog_product_id . "' AND attribute_id = '" . (int)$product_attribute['attribute_id'] . "' AND language_id = '" . (int)$language_id . "'");

						$this->db->query("INSERT INTO " . DB_PREFIX . "blog_product_attribute SET blog_product_id = '" . (int)$blog_product_id . "', attribute_id = '" . (int)$product_attribute['attribute_id'] . "', language_id = '" . (int)$language_id . "', text = '" .  $this->db->escape($product_attribute_description['text']) . "'");
					}
				}
			}
		}
			...
			foreach ($data['product_option'] as $product_option) {
				if ($product_option['type'] == 'select' || $product_option['type'] == 'radio' || $product_option['type'] == 'checkbox' || $product_option['type'] == 'image') {
					if (isset($product_option['product_option_value'])) {
						$this->db->query("INSERT INTO " . DB_PREFIX . "blog_product_option SET blog_product_id = '" . (int)$blog_product_id . "', option_id = '" . (int)$product_option['option_id'] . "', required = '" . (int)$product_option['required'] . "'");

						$blog_product_option_id = $this->db->getLastId();

						foreach ($product_option['product_option_value'] as $product_option_value) {
							$this->db->query("INSERT INTO " . DB_PREFIX . "blog_product_option_value SET blog_product_option_id = '" . (int)$blog_product_option_id . "', blog_product_id = '" . (int)$blog_product_id . "', option_id = '" . (int)$product_option['option_id'] . "', option_value_id = '" . (int)$product_option_value['option_value_id'] . "', quantity = '" . (int)$product_option_value['quantity'] . "', subtract = '" . (int)$product_option_value['subtract'] . "', price = '" . (float)$product_option_value['price'] . "', price_prefix = '" . $this->db->escape($product_option_value['price_prefix']) . "', points = '" . (int)$product_option_value['points'] . "', points_prefix = '" . $this->db->escape($product_option_value['points_prefix']) . "', weight = '" . (float)$product_option_value['weight'] . "', weight_prefix = '" . $this->db->escape($product_option_value['weight_prefix']) . "'");
						}
					}
				} else {
					$this->db->query("INSERT INTO " . DB_PREFIX . "blog_product_option SET blog_product_id = '" . (int)$blog_product_id . "', option_id = '" . (int)$product_option['option_id'] . "', value = '" . $this->db->escape($product_option['value']) . "', required = '" . (int)$product_option['required'] . "'");
				}
			}
		}
		...
		if (isset($data['product_discount'])) {
			foreach ($data['product_discount'] as $product_discount) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "blog_product_discount SET blog_product_id = '" . (int)$blog_product_id . "', customer_group_id = '" . (int)$product_discount['customer_group_id'] . "', quantity = '" . (int)$product_discount['quantity'] . "', priority = '" . (int)$product_discount['priority'] . "', price = '" . (float)$product_discount['price'] . "', date_start = '" . $this->db->escape($product_discount['date_start']) . "', date_end = '" . $this->db->escape($product_discount['date_end']) . "'");
			}
		}

		if (isset($data['product_special'])) {
			foreach ($data['product_special'] as $product_special) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "blog_product_special SET blog_product_id = '" . (int)$blog_product_id . "', customer_group_id = '" . (int)$product_special['customer_group_id'] . "', priority = '" . (int)$product_special['priority'] . "', price = '" . (float)$product_special['price'] . "', date_start = '" . $this->db->escape($product_special['date_start']) . "', date_end = '" . $this->db->escape($product_special['date_end']) . "'");
			}
		}
		...
		if (isset($data['product_category'])) {
			foreach ($data['product_category'] as $category_id) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "blog_product_to_category SET blog_product_id = '" . (int)$blog_product_id . "', blog_category_id = '" . (int)$category_id . "'");
			}
		}
		...
		if (isset($data['product_reward'])) {
			foreach ($data['product_reward'] as $customer_group_id => $product_reward) {
				if ((int)$product_reward['points'] > 0) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "blog_product_reward SET blog_product_id = '" . (int)$blog_product_id . "', customer_group_id = '" . (int)$customer_group_id . "', points = '" . (int)$product_reward['points'] . "'");
				}
			}
		}
		...
		if ($data['keyword']) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "url_alias SET query = 'blog_product_id=" . (int)$blog_product_id . "', keyword = '" . $this->db->escape($data['keyword']) . "'");
		}
	...
	public function editProduct($blog_product_id, $data) {
		...
		if (isset($data['product_option'])) {
			foreach ($data['product_option'] as $product_option) {
				if ($product_option['type'] == 'select' || $product_option['type'] == 'radio' || $product_option['type'] == 'checkbox' || $product_option['type'] == 'image') {
					if (isset($product_option['product_option_value'])) {
						$this->db->query("INSERT INTO " . DB_PREFIX . "blog_product_option SET blog_product_option_id = '" . (int)$product_option['blog_product_option_id'] . "', blog_product_id = '" . (int)$blog_product_id . "', option_id = '" . (int)$product_option['option_id'] . "', required = '" . (int)$product_option['required'] . "'");

						$blog_product_option_id = $this->db->getLastId();

						foreach ($product_option['product_option_value'] as $product_option_value) {
							$this->db->query("INSERT INTO " . DB_PREFIX . "blog_product_option_value SET blog_product_option_value_id = '" . (int)$product_option_value['blog_product_option_value_id'] . "', blog_product_option_id = '" . (int)$blog_product_option_id . "', blog_product_id = '" . (int)$blog_product_id . "', option_id = '" . (int)$product_option['option_id'] . "', option_value_id = '" . (int)$product_option_value['option_value_id'] . "', quantity = '" . (int)$product_option_value['quantity'] . "', subtract = '" . (int)$product_option_value['subtract'] . "', price = '" . (float)$product_option_value['price'] . "', price_prefix = '" . $this->db->escape($product_option_value['price_prefix']) . "', points = '" . (int)$product_option_value['points'] . "', points_prefix = '" . $this->db->escape($product_option_value['points_prefix']) . "', weight = '" . (float)$product_option_value['weight'] . "', weight_prefix = '" . $this->db->escape($product_option_value['weight_prefix']) . "'");
						}
					}
				} else {
					$this->db->query("INSERT INTO " . DB_PREFIX . "blog_product_option SET blog_product_option_id = '" . (int)$product_option['blog_product_option_id'] . "', blog_product_id = '" . (int)$blog_product_id . "', option_id = '" . (int)$product_option['option_id'] . "', value = '" . $this->db->escape($product_option['value']) . "', required = '" . (int)$product_option['required'] . "'");
				}
			}
		}
		...
		if (isset($data['product_recurring'])) {
			foreach ($data['product_recurring'] as $product_recurring) {
				$this->db->query("INSERT INTO `" . DB_PREFIX . "blog_product_recurring` SET `blog_product_id` = " . (int)$blog_product_id . ", customer_group_id = " . (int)$product_recurring['customer_group_id'] . ", `recurring_id` = " . (int)$product_recurring['recurring_id']);
			}
		}
	...
	public function copyProduct($blog_product_id) {
		...
		if ($query->num_rows) {
			...
			$data['product_attribute'] = $this->getProductAttributes($blog_product_id);
			$data['product_description'] = $this->getProductDescriptions($blog_product_id);
			$data['product_discount'] = $this->getProductDiscounts($blog_product_id);
			$data['product_filter'] = $this->getProductFilters($blog_product_id);
			$data['product_image'] = $this->getProductImages($blog_product_id);
			$data['product_option'] = $this->getProductOptions($blog_product_id);
			$data['product_related'] = $this->getProductRelated($blog_product_id);
			$data['product_reward'] = $this->getProductRewards($blog_product_id);
			$data['product_special'] = $this->getProductSpecials($blog_product_id);
			$data['product_category'] = $this->getProductCategories($blog_product_id);
			$data['product_download'] = $this->getProductDownloads($blog_product_id);
			$data['product_layout'] = $this->getProductLayouts($blog_product_id);
			$data['product_store'] = $this->getProductStores($blog_product_id);
			$data['product_recurrings'] = $this->getRecurrings($blog_product_id);
	...
	public function getProduct($blog_product_id) {
		$query = $this->db->query("SELECT DISTINCT *, (SELECT keyword FROM " . DB_PREFIX . "url_alias WHERE query = 'blog_product_id=" . (int)$blog_product_id . "') AS keyword FROM " . DB_PREFIX . "blog_product p LEFT JOIN " . DB_PREFIX . "blog_product_description pd ON (p.blog_product_id = pd.blog_product_id) WHERE p.blog_product_id = '" . (int)$blog_product_id . "' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
	...
	public function getProductAttributes($blog_product_id) {
		...
		$product_attribute_query = $this->db->query("SELECT attribute_id FROM " . DB_PREFIX . "blog_product_attribute WHERE blog_product_id = '" . (int)$blog_product_id . "' GROUP BY attribute_id");

		foreach ($product_attribute_query->rows as $product_attribute) {
			$product_attribute_description_data = array();

			$product_attribute_description_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "blog_product_attribute WHERE blog_product_id = '" . (int)$blog_product_id . "' AND attribute_id = '" . (int)$product_attribute['attribute_id'] . "'");
	...
	public function getProductOptions($blog_product_id) {
		...
		foreach ($product_option_query->rows as $product_option) {
			...
			$product_option_value_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "blog_product_option_value pov LEFT JOIN " . DB_PREFIX . "option_value ov ON(pov.option_value_id = ov.option_value_id) WHERE pov.blog_product_option_id = '" . (int)$product_option['blog_product_option_id'] . "' ORDER BY ov.sort_order ASC");
	...
	public function getTotalProducts($data = array()) {
		$sql = "SELECT COUNT(DISTINCT p.blog_product_id) AS total FROM " . DB_PREFIX . "blog_product p LEFT JOIN " . DB_PREFIX . "blog_product_description pd ON (p.blog_product_id = pd.blog_product_id)";
