...
class ControllerCatalogProduct extends Controller {
	...
	protected function getList() {
		...
		if($user['user_group_id'] == 11){
			...
			$query = $this->db->query("SELECT product_id FROM `" . DB_PREFIX . "product` WHERE model = '" . $this->db->escape($user['username']) . "'");

			$product_id = $query->row['product_id'];
			$this->request->get['product_id'] = $product_id;
			$this->response->redirect($this->url->link('catalog/product/edit', 'token=' . $this->session->data['token'].'&product_id=' . $product_id));
	...
	protected function getForm() {
		...
		if (isset($this->request->post['instagram_id'])) {
			$data['instagram_id'] = $this->request->post['instagram_id'];
		} elseif (!empty($product_info)) {
		...
		if (isset($this->request->post['instagram_code'])) {
			$data['instagram_code'] = $this->request->post['instagram_code'];
		} elseif (!empty($product_info)) {
		...
		if (isset($this->request->post['calendar'])) {
			$data['calendars'] = $this->request->post['calendar'];
		} elseif (isset($this->request->get['product_id'])) {
			$data['calendars'] = $this->model_catalog_product->getProductCalendars($this->request->get['product_id']);
		} else {
		...
		foreach ($product_images as $product_image) {
			...
			$data['product_images'][] = array(
				...
				'desc' => $product_image['desc'],
				'mobil' => $product_image['mobil'],
