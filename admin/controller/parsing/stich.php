...
$brands = $Brand->getBrands();
	...
	<div class="wrapper">
		...
		<h2>Парсинг сайта - http://stich.ru/</h2>
		</div>
	</div>
...
<table>
	<tr>
		<td width="400">
			Сюда URL <b>на список товаров</b> со STICH<br>(!!! Правильно делай выборку! Весь товар из этого списка попадет в одну категорию под один бренд)			
		</td>
		<td>
			<input type="text" class="sturminform_list text" maxlength="100" size="20" />
		</td>
		<td>
	...
	<tr>
		<td>
			Сюда URL <b>на товар</b> со stich
		</td>
		<td>
			<input type="text" class="sturminform text" maxlength="100" size="20" />
		</td>
		<td>
			<a href="javascript:" class="sturminformreload">перечитать</a>
		</td>
	...
	<tr>
		<td>Параметры в размер</td>
		<td colspan="3">[<input type="checkbox" class="wid"> Ширину в размер] [<input type="checkbox" class="hei"> Длину в размер] [<input type="checkbox" class="sto"> Сторону в размер] [<input type="checkbox" class="color"> Определять цвет]</td>
	</tr>
	...
	<tr>
		<td>Назначить цвет</td>
		<td colspan="3"><input type="text" class="color_value" value="" placeholder="Тут пишем цвет"></td>
	</tr>
	<tr>
		<td>Цена</td>
...
<script>
	...
	function parce(key) {
		...
		//console.log(wid+' '+$('.wid').prop('checked'));	
		$.ajax({
			url: 'parsing/ajax_parsing_stich.php',
			type: "POST",
			dataType: 'text',
			data: 'url='+url+'&color='+color+'&color_value='+color_value+'&wid='+wid+'&hei='+hei+'&sto='+sto+'&list='+list+'&artkl='+artkl+'&name='+name+'&price='+price+'&parent='+parent+'&image='+image+'&size='+size+'&category='+category+'&brand='+brand+'&memo='+memo,
			beforeSend: function(){
